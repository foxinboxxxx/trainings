...
class ModifyResourceView(MethodView):
    ...
    def get_update_parameters(self, form_instance):
        ...
        parameter_list = [parameter for parameter in parameter_list
                            if parameter[0] != "_"
                            and parameter not in ["submit", "csrf_token", "meta"]]
        return parameter_list

def register_admin_resource(model, edit_form=None):
    resource_name = model.__name__.lower()
    view_func = ModifyResourceView.as_view(f"{resource_name}",
                                            model=model,
                                            edit_form=edit_form)
    edit_allowed = True
    ...
    admin.add_url_rule(f"/{resource_name}/",
                    view_func=TableView.as_view(
                        f"{resource_name}_table",
                        model=model,
                        edit_allowed=edit_allowed)
                    )
    admin.add_url_rule(f"/{resource_name}/<int:resource_id>",
                        view_func=view_func,
