...
@click.option("-p", "--password", hide_input=True, prompt="Password", default="password123", help="Users password")
@click.option("-a", "--admin", is_flag=True, help="It turns the new user into an admin", prompt="Is this user an admin?")
@with_appcontext
def create(username, email, password, admin):
    ...
    try:
        db.session.add(user)
        db.session.commit()
        click.echo("User {} has been successfully saved to the database.".format(username))
