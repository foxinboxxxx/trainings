...
class BelongsToOtherFieldOption:
    def __init__(self, table, belongs_to, foreign_key=None, message=None):
        ...
        belongs_to - name of field that acts as parent field
        """
        if not table:
...
class ItemForm(FlaskForm):
    title       = StringField("Title", validators=[InputRequired("Input is required"), DataRequired("Data is required"), Length(min=5, max=20, message="Length between 5 and 20")])
...
def item(item_id):
    ...
    if item:
        ...
        return render_template("item.html", item=item, commentForm=commentForm, comments=comments, deleteItemForm=deleteItemForm)
    return redirect(url_for("home"))
...
def home():
    ...
    # Not to submit form with POST method
...
def new_item():
    ...
    # TODO: Apply custom select field which takes automatically all of the choices from DB
    # SelectField class - hint
    c.execute("SELECT id, name FROM categories")
    categories = c.fetchall()
    # [(1, 'Food'), (2, 'Technology'), (3, 'Books')] - numbers are integers
...
def get_db():
    ...
    # g - is object provided by application context
    if db is None:
